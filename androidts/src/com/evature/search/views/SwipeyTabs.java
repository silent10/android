// Based on this blog post: http://blog.peterkuterna.net/2011/09/viewpager-meets-swipey-tabs.html

package com.evature.search.views;

import com.evature.search.R;
import com.evature.search.R.styleable;
...
public class SwipeyTabs extends ViewGroup implements OnPageChangeListener {
	...
	protected void dispatchDraw(Canvas canvas) {
		if (mCurrentPos != -1 && mCurrentTabPos.length > 0) { // TAL: fix to race condition in dynamic tabs
			...
			final float relativePos = 1.0f - Math.min(Math.abs((float) (centerOfTab - center) / (float) (centerDiv3)),
					1.0f);

			mCachedTabPaint.setAlpha((int) (255 * relativePos));
			canvas.drawRect(mCurrentTabPos[mCurrentPos], tabSelectedTop,
					mCurrentTabPos[mCurrentPos] + currentTab.getMeasuredWidth(), tabSelectedTop + mTabIndicatorHeight,
					mCachedTabPaint);
	...
	private int interpolateColor(final int color1, final int color2, float fraction) {
		...
		return Color.argb((int) (alpha * 255.0f), (int) (red * 255.0f), (int) (green * 255.0f), (int) (blue * 255.0f));
	}
