...
// from https://github.com/Todd-Davies/ProgressWheel
// Iftah: modified:
//   default padding to zero
//   code barSizeChanging
/**
...
public class ProgressWheel extends View {
    ...
    private boolean barSizeChange = false;

    //Other
    private String text = "";
    private String[] splitText = {};
	private int barSizeIncrement = 4;
    ...
    public ProgressWheel(Context context, AttributeSet attrs) {
        ...
        parseAttributes(context.obtainStyledAttributes(attrs,
                    R.styleable.EvatureProgressWheel));
    }
    ...
    private void parseAttributes(TypedArray a) {
        ...
        spinSpeed = (int) a.getDimension(R.styleable.EvatureProgressWheel_spinSpeed,
                spinSpeed);

        delayMillis = a.getInteger(R.styleable.EvatureProgressWheel_delayMillis,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(R.styleable.EvatureProgressWheel_barColor, barColor);

        barLength = (int) a.getDimension(R.styleable.EvatureProgressWheel_pwBarLength,
                barLength);

        textSize = (int) a.getDimension(R.styleable.EvatureProgressWheel_textSize,
                textSize);

        textColor = (int) a.getColor(R.styleable.EvatureProgressWheel_textColor,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(R.styleable.EvatureProgressWheel_text)) {
            setText(a.getString(R.styleable.EvatureProgressWheel_text));
        }

        rimColor = (int) a.getColor(R.styleable.EvatureProgressWheel_rimColor,
                rimColor);

        circleColor = (int) a.getColor(R.styleable.EvatureProgressWheel_circleColor,
                circleColor);

        contourColor = a.getColor(R.styleable.EvatureProgressWheel_contourColor, contourColor);
        contourSize = a.getDimension(R.styleable.EvatureProgressWheel_contourSize, contourSize);

        setBarSizeChange(a.getBoolean(R.styleable.EvatureProgressWheel_barSizeChange, false));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //Draw the bar
        if (isBarSizeChange()) {
        	barLength += barSizeIncrement;
        	if (barLength > 360 && barSizeIncrement > 0) {
        		barSizeIncrement *= -1;
        	}
        	if (barLength <= 0 && barSizeIncrement < 0) {
        		barSizeIncrement *= -1;
        		barLength = 1;
        	}
        }
    ...
    */
	...
	public boolean isBarSizeChange() {
		return barSizeChange;
	}

	public void setBarSizeChange(boolean barSizeChange) {
		this.barSizeChange = barSizeChange;
	}
}
