...
import com.softskills.components.S3DrawableBackgroundLoader;
...
public class HotelListAdapter extends BaseAdapter {
	...
	HotelListAdapter(HotelListFragment parent, EvaDatabase evaDb)
	{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		 ...
		 holder.hotelIndex = position;
		 
		 Spanned spannedName = Html.fromHtml(holder.hotel.mSummary.mName);
		 
		 String name = spannedName.toString();
		 
		 Display display = ((WindowManager) mParent.getActivity().getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
...
//		 }
		 ...
		 S3DrawableBackgroundLoader.getInstance().loadDrawable("http://images.travelnow.com" + holder.hotel.mSummary.mThumbNailUrl,
				 							holder.image,
				 							mEvaHotelIcon);
		...
		holder.image.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				String hotelInfo = XpediaProtocol.getExpediaHotelInformation(mEvaDb.mHotelData[holder.hotelIndex].mSummary.mHotelId);
					
				Intent intent = new Intent(mParent,ShowHotel.class);
					
				intent.putExtra("hotelString", hotelInfo);
				intent.putExtra("hotelIndex", holder.hotelIndex );
