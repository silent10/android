...
public class ShowHotelFragment extends Fragment
{
	@Override
	...
	protected static final String TAG = "ShowHotel";
	...
	ShowHotelFragment(int hotelIndex)
	{
...
//		}
		...
		Log.i(TAG,"1)PropertyDescription:"+mHotelData);

		Log.i(TAG,"2)PropertyDescription:"+mHotelData.mDetails);

		Log.i(TAG,"3)PropertyDescription:"+mHotelData.mDetails.propertyDescription);

		Spanned marked_up = Html.fromHtml("<html><body>"+mHotelData.mDetails.propertyDescription+"</body></html>");

		mPropertyDescription.loadData("<font color=\"black\">"+marked_up.toString()+"</font>", "text/html", "utf-8");				
		...
		if(wifiAvailable) count++;
		if(parkingAvailable) count++;
		if(poolAvailable) count++;
		if(breakfestAvailable) count++;

		Integer thumbIds[] = new Integer[count];

		count =0;

		if(wifiAvailable) thumbIds[count++] = R.drawable.internet;
		if(parkingAvailable) thumbIds[count++] = R.drawable.parking;
		if(poolAvailable) thumbIds[count++] = R.drawable.pool;
		if(breakfestAvailable) thumbIds[count++] = R.drawable.breakfest;
	...
	public ShowHotelFragment(String hotelInfo, int hotelIndex) {		
		mHotelIndex =  hotelIndex;
	}

	static ShowHotelFragment newInstance(int hotelIndex)
	{
		return new ShowHotelFragment(hotelIndex);
	}
