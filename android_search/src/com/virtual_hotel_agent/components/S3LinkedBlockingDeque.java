...
/**
 * An optionally-bounded {@linkplain S3BlockingDeque blocking deque} based on linked nodes.
 * 
 * <p>
 * The optional capacity bound constructor argument serves as a way to prevent excessive expansion. The capacity, if
 * unspecified, is equal to {@link Integer#MAX_VALUE}. Linked nodes are dynamically created upon each insertion unless
 * this would bring the deque above capacity.
 * 
 * <p>
 * Most operations run in constant time (ignoring time spent blocking). Exceptions include {@link #remove(Object)
 ...
 */
public class S3LinkedBlockingDeque<E> extends AbstractQueue<E> implements S3BlockingDeque<E>, java.io.Serializable {
	 ...
	 * dequeued Node. That would cause two problems: - allow a rogue Iterator to cause unbounded memory retention -
	...
	/*
	 * We have "diamond" multiple interface/abstract class inheritance here, and that introduces ambiguities. Often we
	 * want the BlockingDeque javadoc combined with the AbstractQueue implementation, so a lot of method specs are
	 * duplicated here.
	 */
	...
	static final class Node<E> {
		...
		/**
		 * One of: - the real predecessor Node - this Node, meaning the predecessor is tail - null, meaning there is no
		 * predecessor
		 */
		Node<E> prev;

		/**
		 * One of: - the real successor Node - this Node, meaning the successor is head - null, meaning there is no
		 * successor
		 */
		Node<E> next;
	 ...
	 */
	public S3LinkedBlockingDeque(int capacity) {
		if (capacity <= 0)
	...
	/**
	 * Creates a {@code LinkedBlockingDeque} with a capacity of {@link Integer#MAX_VALUE}, initially containing the
	 ...
	 */
	public S3LinkedBlockingDeque(Collection<? extends E> c) {
		this(Integer.MAX_VALUE);
	 ...
	 * a capacity-restricted deque, it is generally preferable to use method {@link #offer(E) offer}.
	 * 
	 * <p>
	...
	public E take() throws InterruptedException {
		// Was : return takeFirst(); Changed it to implement dequeue for the thread pool implementation.
		return takeLast();
	}
	...
	/**
	 * Returns the number of additional elements that this deque can ideally (in the absence of memory or resource
	 * constraints) accept without blocking. This is always equal to the initial capacity of this deque less the current
	 ...
	 * 
	 * We don't want to acquire the lock for every iteration, but we also want other threads a chance to interact with
	 * the collection, especially when count is close to capacity.
	 */
	...
	/**
	 * Returns an array containing all of the elements in this deque, in proper sequence; the runtime type of the
	 * returned array is that of the specified array. If the deque fits in the specified array, it is returned therein.
	 ...
	 * from first (head) to last (tail). The returned {@code Iterator} is a "weakly consistent" iterator that will never
	 * throw {@link java.util.ConcurrentModificationException ConcurrentModificationException}, and guarantees to
	 ...
	 * order from last (tail) to first (head). The returned {@code Iterator} is a "weakly consistent" iterator that will
	 * never throw {@link java.util.ConcurrentModificationException ConcurrentModificationException}, and guarantees to
	 * traverse elements as they existed upon construction of the iterator, and may (but is not guaranteed to) reflect
	 * any modifications subsequent to construction.
	 */
	...
	private abstract class AbstractItr implements Iterator<E> {
		...
		/**
		 * nextItem holds on to item fields because once we claim that an element exists in hasNext(), we must return
		 * item read under lock (in advance()) even if it was in the process of being removed when hasNext() was called.
		 */
		E nextItem;

		/**
		 * Node returned by most recent call to next. Needed by remove. Reset to null if this element is deleted by a
		 * call to remove.
		 */
		...
		AbstractItr() {
			...
			final ReentrantLock lock = S3LinkedBlockingDeque.this.lock;
			lock.lock();
			try {
				next = firstNode();
	 ...
	 * 
	 * @serialData The capacity (int), followed by elements (each an {@code Object}) in the proper order, followed by a
	 *             null
	 * @param s
	 *            the stream
	 */
