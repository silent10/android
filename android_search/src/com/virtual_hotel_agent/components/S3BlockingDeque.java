...
/**
 * A {@link S3Deque} that additionally supports blocking operations that wait for the deque to become non-empty when
 * retrieving an element, and wait for space to become available in the deque when storing an element.
 * 
 * <p>
 * <tt>BlockingDeque</tt> methods come in four forms, with different ways of handling operations that cannot be
 * satisfied immediately, but may be satisfied at some point in the future: one throws an exception, the second returns
 * a special value (either <tt>null</tt> or <tt>false</tt>, depending on the operation), the third blocks the current
 * thread indefinitely until the operation can succeed, and the fourth blocks for only a given maximum time limit before
 * giving up. These methods are summarized in the following table:
 * 
 * <p>
 ...
 * 
 * <p>
 * Like any {@link BlockingQueue}, a <tt>BlockingDeque</tt> is thread safe, does not permit null elements, and may (or
 * may not) be capacity-constrained.
 * 
 * <p>
 * A <tt>BlockingDeque</tt> implementation may be used directly as a FIFO <tt>BlockingQueue</tt>. The methods inherited
 * from the <tt>BlockingQueue</tt> interface are precisely equivalent to <tt>BlockingDeque</tt> methods as indicated in
 * the following table:
 * 
 * <p>
 * <table BORDER CELLPADDING=3 CELLSPACING=1>
 ...
 */
public interface S3BlockingDeque<E> extends BlockingQueue<E>, S3Deque<E> {
	/*
	 * We have "diamond" multiple interface inheritance here, and that introduces ambiguities. Methods might end up with
	 * different specs depending on the branch chosen by javadoc. Thus a lot of methods specs here are copied from
	 * superinterfaces.
	 */

	/**
	 * Inserts the specified element at the front of this deque if it is possible to do so immediately without violating
	 * capacity restrictions, throwing an <tt>IllegalStateException</tt> if no space is currently available. When using
	 * a capacity-restricted deque, it is generally preferable to use {@link #offerFirst(E) offerFirst}.
	 * 
	 * @param e
	 *            the element to add
	 ...
	 * capacity restrictions, throwing an <tt>IllegalStateException</tt> if no space is currently available. When using
	 * a capacity-restricted deque, it is generally preferable to use {@link #offerLast(E) offerLast}.
	 * 
	 * @param e
	 *            the element to add
	 * @throws IllegalStateException
	 ...
	 * available. When using a capacity-restricted deque, this method is generally preferable to the
	 * {@link #addFirst(E) addFirst} method, which can fail to insert an element only by throwing an exception.
	 * 
	 * @param e
	 *            the element to add
	 * @throws ClassCastException
	 ...
	 * Inserts the specified element at the end of this deque if it is possible to do so immediately without violating
	 * capacity restrictions, returning <tt>true</tt> upon success and <tt>false</tt> if no space is currently
	 * available. When using a capacity-restricted deque, this method is generally preferable to the {@link #addLast(E)
	 ...
	 * @param unit
	 *            a <tt>TimeUnit</tt> determining how to interpret the <tt>timeout</tt> parameter
	 ...
	 * @param unit
	 *            a <tt>TimeUnit</tt> determining how to interpret the <tt>timeout</tt> parameter
	 * @return the head of this deque, or <tt>null</tt> if the specified waiting time elapses before an element is
	 *         available
	 * @throws InterruptedException
	 *             if interrupted while waiting
	 */
	 ...
	 * @param unit
	 *            a <tt>TimeUnit</tt> determining how to interpret the <tt>timeout</tt> parameter
	 * @return the tail of this deque, or <tt>null</tt> if the specified waiting time elapses before an element is
	 *         available
	 * @throws InterruptedException
	 *             if interrupted while waiting
	 */
	 ...
	 * it is unchanged. More formally, removes the first element <tt>e</tt> such that <tt>o.equals(e)</tt> (if such an
	 ...
	 * it is unchanged. More formally, removes the last element <tt>e</tt> such that <tt>o.equals(e)</tt> (if such an
	 * element exists). Returns <tt>true</tt> if this deque contained the specified element (or equivalently, if this
	 * deque changed as a result of the call).
	 * 
	 * @param o
	 ...
	 * capacity-restricted deque, it is generally preferable to use {@link #offer(E) offer}.
	 * 
	 * <p>
	 * This method is equivalent to {@link #addLast(E) addLast}.
	 * 
	 * @param e
	 *            the element to add
	 * @throws IllegalStateException
	 ...
	 * 
	 * <p>
	 * This method is equivalent to {@link #offerLast(E) offerLast}.
	 * 
	 * @param e
	 *            the element to add
	 * @throws ClassCastException
	 ...
	 * 
	 * <p>
	 * This method is equivalent to {@link #putLast(E) putLast}.
	 * 
	 * @param e
	 *            the element to add
	 * @throws InterruptedException
	 ...
	 * 
	 * <p>
	 * This method is equivalent to {@link #offerLast(E,long,TimeUnit) offerLast}.
	 * 
	 * @param e
	 *            the element to add
	 ...
	 * this deque unless it would violate capacity restrictions.
	 * 
	 * <p>
	 * This method is equivalent to {@link #addFirst(E) addFirst}.
	 * 
	 * @throws IllegalStateException
	 *             {@inheritDoc}
	 * @throws ClassCastException
