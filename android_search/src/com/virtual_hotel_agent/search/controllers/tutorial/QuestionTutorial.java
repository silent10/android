...
public class QuestionTutorial extends BaseTutorial {
	...
	private State state;
	private ChatItem evaQuestionItem;
	private View evaQuestionView;
	
	public QuestionTutorial() {
		super(NAME);
...
//			break;
	...
	public void onAddChatItem(final ChatItem chatItem, View chatView, final ChatFragment chatFragment) {
		...
		if (state == State.NotStarted || state == State.Done) {
			...
			state = State.ShowEvaQuestionItem;
		}
...
//				View chatView = chatFragment.getViewForChatItem(chatItem);
		...
		if (chatItem.getType() != ChatType.DialogQuestion || chatItem.getFlowElement().Type != TypeEnum.Question) {
			// start only on Eva question chat
			return ;
		}
		...
		showcaseView.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				QuestionTutorial.this.onClickShowcase(chatFragment);
			}
		});
	...
	public boolean shouldStart(ChatItem chatItem, View chatView, Activity activity) {
		if (state != State.NotStarted && state != State.Done) {
			...
			// TODO: temp: allow restart (allow state Done)
			return false;
		}
		if (chatItem.getType() != ChatType.DialogQuestion || chatItem.getFlowElement().Type != TypeEnum.Question) {
			// start only on Eva question chat
		...
		if (TutorialController.chatTutorial.getStatus() != TutorialStatus.Played) {
		...
		evaQuestionItem = chatItem;
		evaQuestionView = chatView;
		return true;
	}
}
